  ...
  var Instance = (function () {
    return {
      ...
      addQuestion : function (question) {
        ...
        progress.question.push(question);
        if (!this.getSelectedQuestion()) {
          this.select(this.questions.length-1);
        }
      },
      ...
      addAnswer : function (answer) {
        ...
        progress.answer.push(answer);
        answer.solved = true;

        this.getSelectedQuestion().solved = true;
      ...
      next : function () {
        progress.selectedQuestionIndex = this.selected.questionIndex =
          Math.min(this.selected.questionIndex+1, this.questions.length-1);
      },

      previous : function () {
        progress.selectedQuestionIndex = this.selected.questionIndex =
          Math.max(0, this.selected.questionIndex-1);
      },
  ...
  return {
    create : function (level) {
      ...
      for (i=0; i<progress.question.length; i++) {
        if (!progress.question[i].solved) {
          menu.questions.push(progress.question[i]);
        } else {
          console.log(progress.question[i].text, 'is already solved');
        }
      }
      
      menu.selected = {
        questionIndex : progress.selectedQuestionIndex,
        frame : addSprite(0,0, 'selected-question-frame'),
        level : level,
      };
      console.log('selected question index', menu.selected.questionIndex);
